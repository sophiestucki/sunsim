...
def load_MPS_ATLAS_spectra_lc(self, type='ph'):
    ...
    if type=='sp':
        flux *= black_body(wvp[idx_wv], self.temperature_spot) / black_body(wvp[idx_wv], self.temperature_photosphere) 

    return acd, wvp[idx_wv], flux

def load_STAGGER_spectra_lc(self, type='ph'):
    data = np.load(self.stagger_spectra_folder+'STAGGER_qs_high_res_PHOENIX_grid.npy', allow_pickle=True).item()
    acd = np.fromiter(data.keys(), dtype=float)
    wvp = data[acd[0]]['wav'] 
...
def interpolate_mps(self, type='ph'):
    ...
    if type == 'sp':
        ...
        flux_norm *= black_body(wv, self.temperature_spot) / black_body(wv, self.temperature_photosphere) 
...
def interpolate_stagger(self, type='ph'):
    ...
    if type == 'sp':
        flux = np.array(data[1.0]['intensity'][idx_wv] * black_body(wv, self.temperature_spot) / black_body(wv, self.temperature_photosphere))
    else:
        flux = np.array(data[1.0]['intensity'][idx_wv])
